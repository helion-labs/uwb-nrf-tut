...
static uint64 get_sys_times_u64(void);
static uint16_t final_msg_get_ds(uint8 *ts_field);
...
static int ss_resp_run(uint16_t * peer_distance_in_cm)
{
      ...
      if (status_reg & SYS_STATUS_RXFCG)
        {
          ...
          // Make sure that it's a final message sent from the Initiator 
          rx_buffer[ALL_MSG_SN_IDX] = 0;
          if (memcmp(rx_buffer, tx_fnl_msg, ALL_MSG_COMMON_LEN) == 0)
          {
            *peer_distance_in_cm = final_msg_get_ds(&rx_buffer[FINAL_MSG_DISTANCE_IDX]);           
             return 0;
          }
        } else {
          goto cleanup;
...
static uint16_t final_msg_get_ds(uint8 *ts_field)
{
  uint16_t distance_cm = ts_field[0] + (ts_field[1] << 8);
...
int main_resp(void)
{
  ...
  dwt_setrxtimeout(0xFFFE);    //set timeout to 1ms   
