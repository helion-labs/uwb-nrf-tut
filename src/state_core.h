...
typedef struct {
    ...
    // If non-zero, the period of a loop in ms
    uint32_t loop_timer;
...
typedef struct {
    ...
    // for zephyr based projects, this must be set by the user 
    // queue will be used internally with a generic event receive function
    struct k_msgq * state_queue_input_handle;  
    ...
    //    { state_function_pointer_a, int ticks_a , cleanup_func},
    //    { state_function_pointer_b, int ticks_b , cleanup_func},
    //    ...
    // }
    // 
...
*                      DEFINES
**********************************************************/
#define EVENT_MUX_THREAD_SIZE      (1024)
#define STATE_MACHINE_PRIORITY     (5)
#define EVENT_MULTIPLEXER_PRIORITY (5)
#define GENERIC_QUEUE_TIMEOUT      (2500)
#define INVALID_EVENT              (0xFFFFFFFF)
#define EVENT_QUEUE_MAX_DEPTH      (16)
#define STATE_MUTEX_WAIT           (2500)
#define NULL_STATE                 (0xFFFF)
#define STATE_DONT_LOOP            (0)
#define GENERIC_MESSGE_Q_DEPTH     (10)
